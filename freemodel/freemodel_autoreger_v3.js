...
const SELECTORS = {
  ...
  submitSignup:
    'button:has-text("Sign up"), button:has-text("Continue"), button:has-text("Регистрация"), button[type="submit"]',
...
async function registerOne(index, inviteCode) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < MAX_EMAIL_RETRIES; attempt++) {
      ...
      log(
        `[#${index}] email (попытка ${attempt + 1}): ${email}  [token=${mailbox.token.slice(0, 8)}…]`,
      );
