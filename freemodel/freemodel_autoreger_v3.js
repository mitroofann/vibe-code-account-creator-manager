...
//
// v3: instanttempemail.com через REST API (без браузера для почты).
//
// Отличия от v2:
//   - emailnator (headless Chromium) заменён на чистый fetch к instanttempemail.
//   - почтовый домен @fpklm.com (один, без выбора) — но FreeModel его принимает.
//   - поллинг inbox в 5-10 раз быстрее (нет старта браузера на каждое письмо).
//
// Остальной поток (signup на freemodel.dev, ввод OTP в 6 input-полей,
// API-ключ, реф-код, экспорт сессии, пирамида рефов в .last_invite) — как в v2.
//
// Использование:
...
const ite = require('./lib/instanttempemail');
...
async function registerOne(index, inviteCode) {
    ...
    let email = null;
    let mailbox = null;   // { address, token, expires }
    ...
    try {
        ...
        for (let attempt = 0; attempt < MAX_EMAIL_RETRIES; attempt++) {
            mailbox = await ite.createEmail();
            email = mailbox.address;
            log(`[#${index}] email (попытка ${attempt + 1}): ${email}  [token=${mailbox.token.slice(0,8)}…]`);
            ...
            // Жду OTP — через REST API instanttempemail (без браузера)
            log(`[#${index}] жду OTP (макс ${Math.round(otpWaitMs/1000)}с)...`);
            const got = await ite.waitForOtp(mailbox.token, {
                fromHint: config.EMAIL_FROM_HINT || 'freemodel',
                timeoutMs: otpWaitMs,
                pollMs: config.EMAIL_POLL_MS || 4000,
                log: (m) => log(m),
            });
            ...
            log(`[#${index}] ⚠️ OTP не пришёл за ${Math.round(otpWaitMs/1000)}с, новый email (${attempt + 2}/${MAX_EMAIL_RETRIES})`);
            await page.close().catch(() => {});
            page = null;
        }
...
(async () => {
    ...
    log('════════════════════════════════════════');
    log(`  FREEMODEL AUTOREG v3 (instanttempemail) — ${count} акк(а)`);
